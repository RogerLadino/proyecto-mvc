...
def codigo(id_aula, id_ejercicio, id_codigo):
  ...
  codigoUsuario = consultar_codigo(session['id-usuario'], id_ejercicio)

  if not codigoUsuario:
    insertar_codigo(session['id-usuario'], id_ejercicio)
  
  # Los usuarios que podrá ver el profesor son todos
  if es_profesor(session['id-usuario'], id_aula):
  ...
  usuarios_filtrados = [u for u in usuarios if u['idUsuario'] == aula['idUsuario'] or session['id-usuario'] == u['idUsuario']]
